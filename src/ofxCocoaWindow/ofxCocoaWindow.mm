...
void ofxCocoaWindow :: setupOpenGL( int w, int h, int screenMode ) 
{
    ...
    if (shouldSetupCocoaApp){
        ...
        _width = w;
        _height = h;
        _screenMode = (ofWindowMode)screenMode;
        ofxCocoaDelegate *delegate = [NSApp delegate];
        [delegate initGLWindowWithWidth:_width
                                    height:_height
                                windowMode:_screenMode];
    }
}
...
void ofxCocoaWindow :: runAppViaInfiniteLoop ( ofBaseApp * appPtr ) 
{
    ...
    if (shouldSetupCocoaApp){
        ...
        [[NSApp delegate] launchGLWindow];
    }
}

void ofxCocoaWindow :: setShouldSetupCocoaApp(BOOL shouldSet)
{
    shouldSetupCocoaApp = shouldSet;
}
